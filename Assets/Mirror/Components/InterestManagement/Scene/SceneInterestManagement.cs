...
namespace Mirror
{
    ...
    public class SceneInterestManagement : InterestManagement
    {
        ...
        {
            foreach (Transform transformChild in identity.transform)
            {
                if (transformChild.CompareTag("InterestParent"))
                {
                    transformChild.gameObject.SetActive(visible);
                }
            }

            base.SetHostVisibility(identity, visible);

            foreach (Canvas rend in identity.GetComponentsInChildren<Canvas>())
            {
